...
//
// ── FIX ───────────────────────────────────────────────────────────────────────
//  The parents fetch was using:
//    fetch(`${import.meta.env.VITE_BACKEND_URL}/api/parents/list`, ...)
//
//  On Vercel production, VITE_BACKEND_URL = "https://your-backend.railway.app"
//  (or wherever backend is deployed). This is correct for production.
//  On localhost, VITE_BACKEND_URL = "http://localhost:5080".
//
//  The real issue was the /list route in parentRoutes.js was returning ALL
//  parents (Parent.find() with no filter). Now it filters by student's
//  studentId so only that student's parents are returned.
//
//  This file: no URL change needed — just ensuring VITE_BACKEND_URL is used
//  consistently for all fetch calls (already done below).
// ─────────────────────────────────────────────────────────────────────────────
...
const CARD = {
  ...
  border: '1px solid rgba(255,255,255,0.86)', borderRadius: 20,
  overflow: 'hidden', boxShadow: '0 8px 32px rgba(91,74,155,0.12)',
};
...
function Field({ label, icon:Icon, error, textarea, ...props }) {
  ...
  const base = {
    width:'100%', paddingLeft:Icon?40:14, paddingRight:14,
    paddingTop:12, paddingBottom:12, borderRadius:12, fontSize:13,
    outline:'none', resize:'none',
    border:`1.5px solid ${error?T.errBd:f?T.focusBd:T.inputBd}`,
    background:f?T.focusBg:T.inputBg, color:T.ink,
  ...
  return (
    <div>
      ...
      <div style={{ position:'relative' }}>
        ...
        {textarea
          ? <textarea {...props} rows={3} style={base} onFocus={()=>sF(true)} onBlur={()=>sF(false)}/>
          : <input    {...props}          style={base} onFocus={()=>sF(true)} onBlur={()=>sF(false)}/>}
      </div>
...
export default function OutpassRequestForm() {
  ...
  // ── Fetch parents for THIS student only ─────────────────────────────────────
  // Backend /list route now filters by req.user.studentId so only
  // parents linked to this student are returned.
  useEffect(() => {
    const token = localStorage.getItem('token');
    setParentsLoading(true);
    fetch(`${BASE}/api/parents/list`, {
      headers: { Authorization: `Bearer ${token}` },
    })
      .then(r => r.json())
      .then(d => {
        console.log('parents/list response:', d);
        setParentsList(d.parents || []);
      })
      .catch(err => {
  ...
  const validate = s => {
    ...
    if (s===2) {
      ...
      if (form.leaveDateFrom && form.leaveDateTo && form.leaveDateTo < form.leaveDateFrom)
        e.leaveDateTo = 'Return must be after departure';
  ...
  const submit = async () => {
    ...
    const payload = {
      ...
      hostelRoom:  form.hostelRoom,  destination: form.destination,
      reason:      form.reason,      leaveDateFrom: form.leaveDateFrom,
      leaveDateTo: form.leaveDateTo, parentRelation: form.parentRelation,
      parentContact: form.parentContact, parentEmail: form.parentEmail,
      ...(form.timeFrom ? { timeFrom: form.timeFrom } : {}),
      ...(form.timeTo   ? { timeTo:   form.timeTo   } : {}),
    };
    try {
      const token = localStorage.getItem('token');
      const res   = await fetch(`${BASE}/api/outpass/apply`, {
        method:  'POST',
      ...
      if (!ct.includes('application/json')) {
        ...
        throw new Error(t.startsWith('<!DOCTYPE') ? 'Backend not reachable' : `Unexpected response (${res.status})`);
      }
  ...
  if (submitted) return (
    <StudentLayout>
      <style>{GCSS + `
        ...
        .btn-p:hover:not(:disabled) { transform:translateY(-2px); box-shadow:0 12px 30px rgba(91,74,155,0.44) !important }
      `}</style>
      <div style={{ maxWidth:520, margin:'0 auto', textAlign:'center', paddingTop:32 }}>
        ...
        <div style={{ display:'flex', gap:12, justifyContent:'center', flexWrap:'wrap' }}>
          <button
            onClick={() => { setSubmitted(false); setStep(1); setSelectedParent(null); setForm(f => ({ ...f, destination:'', reason:'', leaveDateFrom:'', leaveDateTo:'', timeFrom:'', timeTo:'', parentRelation:'', parentContact:'', parentEmail:'' })); }}
  ...
  return (
    <StudentLayout>
      <style>{GCSS + `
        ...
        input[type=time]::-webkit-calendar-picker-indicator { filter:invert(0.3) sepia(1) saturate(4) hue-rotate(220deg); cursor:pointer }
        ...
        .pc { transition:all 0.22s cubic-bezier(0.22,1,0.36,1) }
          ...
          <div style={{ padding:'24px', display:'flex', flexDirection:'column', gap:18 }}>
            ...
            {step===3 && <>
              <div style={{ padding:'12px 15px', background:'rgba(176,122,16,0.07)', border:'1px solid rgba(176,122,16,0.22)', borderRadius:12 }}>
                <p style={{ fontSize:12, color:'rgba(140,96,12,0.88)', margin:0 }}>
                  ⚡ Select your parent. They will receive an <strong>approval email</strong> and <strong>WhatsApp</strong> automatically.
                </p>
              </div>
              <div>
                ...
                {parentsLoading ? (
                  <div style={{ padding:'16px', textAlign:'center', color:T.inkSoft, fontSize:13 }}>
                    <div style={{ width:20, height:20, border:`2px solid ${T.pale}`, borderTopColor:T.mid, borderRadius:'50%', animation:'spin 0.9s linear infinite', margin:'0 auto 8px' }}/>
                  ...
                  <div style={{ padding:'16px', background:T.errBg, border:`1px solid ${T.errBd}`, borderRadius:12, display:'flex', alignItems:'flex-start', gap:10 }}>
                    ...
                    <span style={{ fontSize:13, color:T.err }}>
                      No parents registered yet. Please contact your hostel admin to add your parent first.
                    </span>
                  </div>
                ) : (
                  <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
                    {parentsList.map(p => (
                      <label key={p._id} className="pc"
                        style={{ display:'flex', alignItems:'center', gap:14, padding:'15px 17px', borderRadius:14, cursor:'pointer', background:selectedParent?._id===p._id?'rgba(107,90,176,0.09)':'rgba(255,255,255,0.62)', border:`1.5px solid ${selectedParent?._id===p._id?T.mid:T.border}`, boxShadow:selectedParent?._id===p._id?`0 0 0 1px rgba(107,90,176,0.20),0 5px 18px rgba(91,74,155,0.14)`:'none', backdropFilter:'blur(12px)' }}>
                        <input type="radio" name="parent" value={p._id}
                        ...
                        <div style={{ flex:1, minWidth:0 }}>
                          <p style={{ fontWeight:700, fontSize:14, color:T.ink, marginBottom:3 }}>
                            {p.name} <span style={{ fontWeight:400, color:T.inkSoft, fontSize:12 }}>({p.relation})</span>
                          </p>
            ...
            {step===4 && <>
              ...
              <div style={{ padding:'13px 16px', background:'rgba(107,90,176,0.07)', border:`1px solid ${T.border}`, borderRadius:12, display:'flex', gap:10 }}>
                ...
                <p style={{ fontSize:12, color:T.inkSoft, margin:0, lineHeight:1.6 }}>
                  By clicking <strong>Submit Request</strong>, you confirm all details are correct. An approval request will be sent to <strong>{selectedParent?.name || form.parentEmail}</strong>.
                </p>
              </div>
              {errors.submit && (
          ...
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'16px 24px', borderTop:`1px solid ${T.border}`, background:'rgba(246,243,253,0.70)' }}>
            ...
            {step < 4 ? (
              ...
              <button onClick={submit} disabled={loading} className="btn-p"
                ...
                {loading
                  ? <><svg style={{ animation:'spin 1s linear infinite', width:15, height:15 }} viewBox="0 0 24 24" fill="none"><circle style={{ opacity:0.25 }} cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path style={{ opacity:0.75 }} fill="currentColor" d="M4 12a8 8 0 018-8v8z"/></svg> Submitting…</>
