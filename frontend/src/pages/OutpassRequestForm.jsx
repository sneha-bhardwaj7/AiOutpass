...
export default function OutpassRequestForm() {
  ...
  const validateStep = (s) => {
    ...
    if (s === 3) {
      if (!form.parentRelation.trim())
        e.parentRelation = "Required";
      if (!/^\d{10}$/.test(form.parentContact))
        e.parentContact = "Enter a valid 10-digit number";
      if (!form.parentEmail.trim())
        e.parentEmail = "Required — approval email will be sent here";
      ...
      if (!contentType.includes("application/json")) {
        ...
        throw new Error(
          text.startsWith("<!DOCTYPE")
            ? "Backend not reachable. Is Express running on port 5000? Does vite.config.js have the /api proxy?"
  ...
  const reviewRows = [
    ...
    { icon: FiUser,          label: "Parent Relation", val: form.parentRelation },
            ...
            {step === 3 && (
              <>
                <div className="p-3 bg-amber-50 border border-amber-100 rounded-xl text-xs text-amber-800">
                  ⚡ On submit, your parent gets an <strong>approval email</strong>, <strong>WhatsApp</strong> message, and <strong>voice call</strong> via n8n.
                </div>
                <Field label="Relation *"        icon={FiUser}  placeholder="e.g. Father, Mother, Guardian" value={form.parentRelation} onChange={set("parentRelation")} error={errors.parentRelation} />
                <Field label="WhatsApp Number *"  icon={FiPhone} type="tel" placeholder="10-digit number" maxLength={10} value={form.parentContact} onChange={set("parentContact")} error={errors.parentContact} />
                <Field label="Parent Email *"     icon={FiMail}  type="email" placeholder="parent@email.com" value={form.parentEmail} onChange={set("parentEmail")} error={errors.parentEmail} />
              </>
            )}
            ...
            {step === 4 && (
              <>
                ...
                <div className="p-3 bg-green-50 border border-green-100 rounded-xl text-xs text-green-800 flex gap-2">
                  ...
                  An approval email will be sent to <strong>{form.parentEmail}</strong> immediately.
                </div>
                {errors.submit && (
